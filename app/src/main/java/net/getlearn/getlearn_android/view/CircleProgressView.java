...
public class CircleProgressView extends View {
    ...
    private int mProgressColor = Color.parseColor("#4ebaff");//进度条颜色
    private int locationStart;//起始位置
    private float startAngle;//开始角度
    private ValueAnimator mAnimator;
    private int direction;//方向，1表示顺时针，-1表示逆时针
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        direction = typedArray.getColor(R.styleable.CircleProgressView_direction, 1);
        typedArray.recycle();
...
//        canvas.drawArc(rectF, startAngle, sweepAngle, false, mProgressPaint);
        //逆时针绘制
//        float sweepAngle =  -1 * 360 * mCurrent / 100 * direction;
//        canvas.drawArc(rectF, startAngle, sweepAngle, false, mProgressPaint);

        //绘制当前进度
        float sweepAngle =  360 * mCurrent / 100 * direction;
        canvas.drawArc(rectF, startAngle, sweepAngle, false, mProgressPaint);
    }
